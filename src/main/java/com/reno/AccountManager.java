...
public class AccountManager {
	...
	final static Logger logger = Logger.getLogger(ClassName.class);
	    ...
	    public Account transferFund(@Context UriInfo uriInfo){
	    	...
	    	logger.info("This is info : " + " Transfer Fund Request fromAccountNumber "+fromAccountNumber + " toAccountNumber "+toAccountNumber+ " depositAmmount"+depositAmmount);
	    	if(null!=accountMap && accountMap.containsKey(fromAccountNumber) && accountMap.containsKey(toAccountNumber)){
	    		...
	    		if(depositAmmount<=accountMap.get(fromAccountNumber).getAacountBalance()){
	    			depositBalance.setAacountBalance(accountMap.get(fromAccountNumber).getAacountBalance() - depositAmmount);
	    			transferFlag = true;
	    		}
				...
				for (Map.Entry<String, Account> entry : accountMap.entrySet()) {
					logger.info("This is info : " + "Account Number : " + entry.getKey() + " Account  Holder : " + entry.getValue().getAccountHolder()+ " Account  Balance : " + entry.getValue().getAacountBalance());
					logger.info("This is info : " + "Transaction History " + entry.getValue().getTransactionHistory().toString());
    ...
    public Account depositFund(@Context UriInfo uriInfo){
    	...
    	if(null!=accountMap && accountMap.containsKey(accountNumber)){
    		...
    		updateBalance.setAacountBalance(Double.parseDouble(uriInfo.getQueryParameters().get("deposit").get(0).toString()));
